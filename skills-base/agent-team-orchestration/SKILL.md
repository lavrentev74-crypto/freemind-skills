...
description: Orchestrate multi-agent teams with roles, task lifecycles, and review workflows. Use when a task requires 2+ specialized agents, parallel workstreams, handoff protocols between agents, or quality gates. Triggers: "нужна команда агентов", "multi-agent", "запустить агентов параллельно", "orchestrate agents". Do NOT use for single-agent tasks — just use Agent tool directly.
---
